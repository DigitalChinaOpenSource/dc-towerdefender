class Enemy {
    ...
    // enemy_img传入敌人的图片url
    constructor(hp, speed, size, enemy_img, money, kill_blood, enemy_type) {
    ...
    move(order) {
        var x_d;
        var y_d;
        if (this.flag == 0 && this.frozen_d == 0) {
            // 切换怪物图片 实现动态变化
    ...
    monster_effect(bullet_type, damage){
        switch(bullet_type){
            case 8: // 子弹类型8 减速20% 持续3秒
                ...
                break;
            case 9:// 子弹类型9 减速35% 持续3秒
                this.reduce_hp(damage);
                this.slowdown = 1;
                this.slowdown_rate = 0.65;
                this.slowdown_start = new Date().getTime();
                this.slowdown_time = 3000;
                break;
